...
public class CertificateService {
    ...
    public OcspWrapper getOcsp(X509Certificate certificate) throws OCSPException {
        String sigAlgOid = certificate.getSigAlgOID();
        CertificateWrapper cert = new CertificateWrapper(certificate);
        CertificateWrapper issuerCert = caService.getRootCertificateFor(cert).orElse(null);
        X509Certificate caCert = issuerCert.getX509Certificate();
        OcspWrapper ocspWrapper = ocspService.buildOcspRequestESEDO(cert.getX509Certificate().getSerialNumber(), caCert);
        return ocspWrapper;
    }
}
